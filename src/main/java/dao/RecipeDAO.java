...
public class RecipeDAO extends DBConnPool {
  ...
  public List<RecipeObj> getThemeRecipeList(Integer search_id) {
    ...
    String sql =
      "SELECT ROWNUM as num, recipe_id, title, subtitle, thumbnail FROM (" +
      ...
      "INNER JOIN recipetag t ON r.recipe_id=t.recipe_id WHERE t.tag_id='" +
      search_id +
      "') a " +
  ...
  public List<RecipeObj> getHotRecipeList() {
    ...
    String sql =
      "SELECT ROWNUM, a.* FROM (SELECT r.recipe_id, r.thumbnail, r.title, r.subtitle, r.difficulty, r.time, r.created_at, r.updated_at, r.state, " +
      "ROUND(AVG(s.score),1) AS star_score, count(*) AS popular FROM recipe r " +
      "LEFT OUTER JOIN star s ON r.recipe_id = s.recipe_id " +
      "LEFT OUTER JOIN bookmark b ON r.recipe_id = b.recipe_id " +
    ...
    try {
      ...
      while (rs.next()) {
        ...
        if (rs.getString("star_score") == null) {
          recipe.setStar_score("0.0");
        } else {
  ...
  public List<Map<String, String>> getRecipeList(
    ...
    String sql =
      "SELECT ROWNUM, recipe_id, title, subtitle, thumbnail, difficulty, time, star_score " +
    ...
    sql +=
      "GROUP BY r.recipe_id, r.title, r.subtitle, r.thumbnail, r.difficulty, r.time, r.created_at" +
  ...
  public List<Map<String, Object>> getSearchList(
    ...
    String sql =
      "SELECT ROWNUM, recipe_id, title, subtitle, thumbnail, difficulty, time, created_at, state FROM (" +
  ...
  public RecipeObj getRecipe(Integer recipe_id) {
    ...
    try {
      ...
      //---별점----------------------------------------------------------------
      String star_sql =
        "SELECT ROUND(AVG(score),1) as star_score FROM star " +
        "WHERE recipe_id = " +
        recipe_id +
        " " +
        "GROUP BY recipe_id";
      ...
      //---만드는 법----------------------------------------------------------------
      String detail_sql =
        "SELECT * FROM recipedetail WHERE recipe_id = " +
        recipe_id +
        " ORDER BY detail_id";
      ...
      for (String name : main_category) {
        String sub_category = "";
        String sub_category_sql =
          "SELECT rc.category_id, rc.recipe_id, rc.sub_category, c.name FROM recipecategory rc " +
      ...
      String ingredient_group_sql =
        "SELECT ingredient_group FROM ingredient WHERE recipe_id = " +
        recipe_id;
      ...
      for (String group_name : ingredient_group) {
        String ingredient_sql =
          "SELECT * FROM ingredient WHERE recipe_id = 1 AND ingredient_group = '" +
          group_name +
          "'";
        rs = stmt.executeQuery(ingredient_group_sql);
  ...
  public Boolean setBookmark(String recipe_id, String user_id) {
    ...
    String sql =
      "SELECT * FROM bookmark WHERE recipe_id = " +
      recipe_id +
      " AND user_id = " +
      user_id;
    ...
    try {
      ...
      if (rs.next()) {
        String bookmark_sql =
          "DELETE FROM bookmark WHERE recipe_id = " +
          recipe_id +
          " AND user_id = " +
          user_id;
        ...
        String bookmark_sql =
          "INSERT INTO bookmark VALUES (" + user_id + ", " + recipe_id + ")";
        stmt.executeUpdate(bookmark_sql);
        result = true;
      }
    } catch (Exception e) {
  ...
  public Boolean createRecipe(RecipeObj recipe) {
    ...
    try {
      ...
      String recipe_sql =
        "INSERT INTO recipe(recipe_id,user_id,title,thumbnail,difficulty,time,subtitle,discription) " +
      ...
      for (Map<String, String> detail : recipe.getDetail()) {
        String detail_sql =
          "INSERT INTO recipedetail(detail_id, recipe_id, description, photo) VALUES (?, ?, ?, ?)";
      ...
      for (String key : recipe.getCategory().keySet()) {
        String main_category_sql =
          "SELECT category_id FROM category WHERE name = '" + key + "'";
        String category_id = "";
      ...
      for (String group_key : recipe.getIngredient().keySet()) {
        ...
        psmt.setString(
          3,
          recipe.getIngredient().get(group_key).get("quantity")
        );
      ...
      if (
        recipe_result && detail_result && category_result && ingredient_result
      ) {
        result = true;
      }
    } catch (Exception e) {
  ...
  public Map<String, List<String>> getCategory() {
    ...
    List<String> main_category = new ArrayList<String>();
    String category_sql = "SELECT category_id, name FROM category";
  ...
  public List<Map<String, Object>> getMyRecipe(
    String user_id,
    Integer offset,
    Integer limit
  ) {
    ...
    String sql =
      "SELECT ROWNUM as num, recipe_id, user_id, thumbnail, title, subtitle, created_at, updated_at, star_score, reple_count FROM (" +
